...
void save_bmp(char *src);//保存BMP图片
void show_video(unsigned char *output_ptr,int x,int y,int image_width,int image_height);//显示视频
 ...
 */
 int jpeg_compress(unsigned char *rgb,int width,int height)
 {
	...
	/*libjpeg库函数，libpng，libgif*/
     ...
     cinfo.input_components = 3;  //压缩的质量
     
     //输入数据格式为RGB
     cinfo.in_color_space = JCS_RGB;
	 
     jpeg_set_defaults(&cinfo); //设置信息
